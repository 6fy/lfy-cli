---
name: lfy-contact
description: 联系人技能。适用于分页查询联系人列表，按 GTM、姓名模糊搜索、按负责人筛选。当用户需要查看联系人列表、按业务线检索联系人时使用此技能。
version: 1.0.0
metadata:
  requires:
    bins: ["lfy-cli"]
...
分页查询当前用户 list 权限范围内的联系人，支持按 GTM、姓名（ILIKE 不区分大小写）、负责人过滤。lfy-cli-server 已剥 `data`，CLI 直接见 `{name, total, contacts}`。
...
2. 执行 `contact get_list`，按需设置 `gtm_id`、`contacts_name`、`sales_ids`、`page`、`page_size`
