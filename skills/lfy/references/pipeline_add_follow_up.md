...
| `content` | string | 必填，跟进记录内容；**须以 `<p></p>` 包裹**（如 `<p>已与客户确认方案，下周签约</p>`），trim 后非空，最多 10000 字符 |
...
| 无权限 | `您暂无权限` |
| 商机不存在 | `商机不存在` |
| 参数非法 | `参数错误` / `参数错误：pipeline_id 错误` / `参数错误：content 不能为空` / `参数错误：content 不能超过 10000 个字符` |
| 其它服务端错误 | `创建失败，请稍后重试` |
