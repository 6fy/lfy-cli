# get_tasks_anytime — 获取指定日期区间任务

## 接口说明

查询调用方指定的日期区间（`start_date` ~ `end_date`，北京时区，含首尾，**最长 60 个自然日**）内的任务列表。主查询一次 SQL 拿任务主行（含 `status_name` / `status_color` / `priority_color` 等），再按 `task_id` **批量**查负责人与任务标签，最终装配为带 `date_key` / `owners` / `tags` 的结构。排序 `pd_date ASC, task_id ASC`。

**不支持周期任务**：与 `get_recent_tasks`、`get_current_week` 一致，仅非周期性任务；不处理周期/重复任务序列。
...
| 参数名         | 类型      | 必填 | 默认 | 说明 |
| -------------- | --------- | ---- | ---- | ---- |
| `start_date`   | string    | 是   | —    | 区间起点 `YYYY-MM-DD`（北京时区语义） |
| `end_date`     | string    | 是   | —    | 区间终点 `YYYY-MM-DD`（含当日）；与 `start_date` 间隔不得超过 **60 个自然日**（含首尾） |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `name` | string | 固定 `"指定日期区间任务"` |
| `start_date` | string | **回显请求**中的区间起点 `YYYY-MM-DD` |
| `end_date` | string | **回显请求**中的区间终点 `YYYY-MM-DD` |
...
`sales_ids=[]` 时**完全不做负责人过滤**（返回区间内全公司任务）；`sales_ids` 非空时按 `bp_task_collaborator.user_id IN (sales_ids) AND user_type = 2` 多人精确匹配。不走 `per_user` 权限表。
...
| 文案 | 含义 |
| ---- | ---- |
| `日期区间不能超过60天` | `end_date` 与 `start_date` 间隔（含首尾）超过 60 个自然日 |
| `结束日期不能早于开始日期` | `end_date < start_date` |
